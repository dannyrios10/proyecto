	...
	function login($con, $usuario, $password)
	{
      ...
      if($result!=null){
		...
		return $usuario;
			...
			function administrador($con, $usuario_ad, $password_ad)
	{
	  require_once 'admin.php';
	  $query = "select * from administradores where usuario_admin = '".$usuario_ad."' and contrasenia_admin = '".$password_ad."'";
      $result=$con->query($query);
      $result->setFetchMode(PDO::FETCH_NUM);
      $result = $result->fetchAll();
      ...
      if($result!=null){
        $result = $result[0];  
        $usuario_ad = new Admin($result[0], $result[1], $result[2], $result[3]);
		return $usuario_ad;
		}
